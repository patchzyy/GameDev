...
namespace TheCure.Mobs
{
    public class LivingEntity : GameObject
    {
        ...
        protected void SetAnimation(string textureName, int frameCount, float frameRate, bool isLooping)
        {
            if (_currentAnimation == textureName)
                return;

            _currentAnimation = textureName;
            SwitchAnimation(textureName, frameCount, frameRate, isLooping);
        }
        ...
        protected void UpdateFacingDirection(Vector2 movementDirection)
        {
            if (movementDirection.LengthSquared() > 0.0001f)
                _facingDirection = Vector2.Normalize(movementDirection);
        }
    }
}