...
namespace TheCure
{
    public class HUD
    {
        ...
        private void DrawHealthBar(SpriteBatch spriteBatch, GameManager gm)
        {
            ...
            Vector2 barPosition = new Vector2(120, 15);

            spriteBatch.Draw(gm.DummyTexture, new Rectangle((int)barPosition.X, (int)barPosition.Y, barWidth, barHeight), Color.Gray);
            float healthRatio = gm.Player.Health / Player.MaxHealth;
            spriteBatch.Draw(gm.DummyTexture, new Rectangle((int)barPosition.X, (int)barPosition.Y, (int)(barWidth * healthRatio), barHeight), Color.Green);
        }
